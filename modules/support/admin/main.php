 ...
 * @Createdate Wed, 20 Aug 2014 07:51:55 GMT
 */
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'title LIKE :q_title OR idgroup LIKE :q_idgroup OR phone LIKE :q_phone OR email LIKE :q_email OR skype_item LIKE :q_skype_item OR skype_type LIKE :q_skype_type OR yahoo_item LIKE :q_yahoo_item' );
	}
...
$array_select_idgroup = array();
	$rows ='SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_support_group' ;
	...
	foreach( $result as $val)
	{
		$xtpl->assign( 'OPTION', array(
			'key' => $val['id'],
			'title' => $val['title'],
			'selected' => ($val['id'] == $row['idgroup']) ? ' selected="selected"' : ''
		) );
		$xtpl->parse( 'main.select_idgroup' );
	}
	
	/*---code them type-skype --*/
	
	
	$array_select_skype_type = array('balloon', 'bigclassic', 'smallclassic', 'smallicon', 'mediumicon');

	foreach( $array_select_skype_type as $title )
		{
		$xtpl->assign( 'OPTION', array(
			'key' => $title,
			'title' => $title,
			'selected' => ($title == $row['skype_type']) ? ' selected="selected"' : ''
		) );
...
$array_select_yahoo_type = array();
	...
	for($i=0; $i<=24; $i++){
		$xtpl->assign( 'OPTION', array(
			'key' => $i,
			'title' => $i,
			'selected' => ($i == $row['yahoo_type']) ? ' selected="selected"' : ''
		) );
